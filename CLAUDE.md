...
| BUG-004 | HIGH | Forpsi/Synology Cyrus IMAP `BrogiASIST/HOTOVO` neexistuje, MOVE tiše selže | **FIXED 2026-05-04** (commit 54a974f, per-host `_brogi_path()`) |
| BUG-005 | HIGH | `email_messages.folder` se aktualizuje I PŘI selhaném IMAP MOVE | **FIXED 2026-05-04** (commit 54a974f, `_uid_move` raise při fail) |
...
| 2026-05-04 | 1.7 | **BUG-004/005** FIXED (commit 54a974f, per-host `_brogi_path` + `_uid_move` raise). **BUG-006** FIXED (commit 7a55330+430f82a, audit script + 32/34 emailů flagged `imap_lost=TRUE`). **BUG-010** prep (commit 3304f93, `X-Brogi-Auto` extrakce v ingest pro budoucí M1). Migrace **019** `imap_lost`. Lekce **#52** (psycopg2 `%` v LIKE → `%%`). |
| 2026-05-04 | 1.8 | **BUG-010 FIXED** plný M1 wire — `smtp_send.py` modul, `2thanks` (deterministický „Díky"), `2reply` text-input state machine (migrace 020 `tg_pending_replies` TTL 30min + `/cancel`), `2cal+Accept` (CAL event + reply pozvateli). 5/5 SMTP probe OK na všech providerech. Commits 99add56→48a5f03→548647a→dcf6626. Lekce **#53–#54**. **Zero open BUGs.** |
| 2026-05-04 | 1.9 | **VERSION feature** — `VERSION` soubor v repo rootu (single source of truth), dashboard nav badge teď ukazuje `v{{ app_version }}` dynamicky (jako BrogiMAT v3.0-dev). Bind mount `./VERSION:/app/VERSION:ro` v docker-compose — bump verze = edit VERSION + restart dashboard, žádný rebuild. Commit a948527. Lekce **#55**. |
| 2026-05-06 | 2.0 | **AI nikdy nerozhoduje is_spam** (Pavlovo strukturní rozhodnutí po Škoda incidentu). Llama prompt: odstraněn `is_spam` field. classify_emails: `is_spam = False` vždy z AI, ignoruje. `_claude_verify_spam` deprecated. **Akce `add_contact` + TG button `📇 2contact`** (pre-row pokud sender NENÍ v Apple Contacts) — Bridge `/contacts/add` přidá sender do skupiny BROGI = trusted whitelist. Spam = JEN explicit Pavlovo klik 2spam / classification_rules / decision_rules manuál. Lekce **#56**. |
| 2026-05-06 | 2.1 | VERSION bump 2.0 → 2.1 (commit 6b313ef). Apple Contacts BROGI naplněna: ŠKODA Auto (4 emaily), ŠKODA Connect (2), iDoklad (2 + logo). 4 false-positive spam emaily vráceny (3 Škoda + 1 iDoklad), Claude verdikty smazány. PROD ověřen: scheduler/dashboard/postgres/chromadb/ollama healthy + Apple Bridge live. |
